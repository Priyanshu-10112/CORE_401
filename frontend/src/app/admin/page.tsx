                ...
                {statsLoading ? (
                    ...
                    [
                        { 
                            ...
                            sub: (stats?.todayEarnings || 0) > 0 ? `₹${stats?.todayEarnings} earned today` : "No earnings today", 
                            icon: IndianRupee, 
                            color: "bg-green-500", 
                        ...
                        { 
                            ...
                            sub: (stats?.newOrders || 0) > 0 ? `${stats?.newOrders} orders pending` : "No pending orders", 
                            icon: Package, 
                            color: "bg-blue-500", 
                        ...
                        { 
                            ...
                            sub: (stats?.pendingPrescriptions || 0) > 0 ? `${stats?.pendingPrescriptions} prescriptions to verify` : "No pending prescriptions", 
                            icon: FileText, 
                            color: "bg-purple-500", 
                        ...
                        { 
                            ...
                            sub: (stats?.expiryAlerts || 0) > 0 ? `${stats?.expiryAlerts} medicines out of stock` : "All medicines in stock", 
                            icon: AlertCircle, 
                            color: "bg-red-500", 
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 border border-slate-100">
                                ...
                                <div className="text-right">
                                    <p className="font-black text-slate-900">{stats?.totalMedicines || 0}</p>
                            ...
                            <div className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 border border-slate-100">
                                ...
                                <div className="text-right">
                                    <p className="font-black text-slate-900">{stats?.rxRequiredMedicines || 0}</p>
                                    <p className="text-[10px] uppercase font-black text-orange-500">Rx Only</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="space-y-8">
                    <div className="p-8 rounded-[2.5rem] bg-indigo-50 border border-indigo-100 h-full relative overflow-hidden">
                        ...
                        <div className="space-y-4 relative z-10">
                            <Button className="w-full h-12 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold shadow-lg shadow-indigo-500/20">
                                <Package className="mr-2 h-4 w-4" />
                                Add New Medicine
                            </Button>
                            <Button variant="outline" className="w-full h-12 rounded-xl border-indigo-200 text-indigo-700 hover:bg-indigo-50 font-bold">
                        ...
                        <div className="mt-8 pt-8 border-t border-indigo-200/50 relative z-10">
                            ...
                            <div className="space-y-2">
                                <div className="flex justify-between text-xs font-bold text-indigo-700 uppercase tracking-widest">
                                    ...
                                    <span>{stats?.inStockMedicines || 0}</span>
                                </div>
                                <div className="h-2 bg-indigo-200 rounded-full overflow-hidden">
                                    <div 
                                        className="h-full bg-indigo-500 rounded-full transition-all duration-500" 
                                        style={{ 
                                            width: `${stats?.totalMedicines ? (stats.inStockMedicines / stats.totalMedicines) * 100 : 0}%` 
                                        }}
                                    />
                                </div>
                                <p className="text-[10px] text-indigo-400 font-medium mt-2">
                                    {stats?.totalMedicines ? Math.round((stats.inStockMedicines / stats.totalMedicines) * 100) : 0}% of medicines are in stock
                                </p>
                            </div>
                        </div>
                    </div>
