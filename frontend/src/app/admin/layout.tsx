...
const NAV_ITEMS = [
    ...
    { label: "Store Profiling", icon: Settings, href: "/admin/profile" },
];
...
export default function AdminLayout({
    ...
    console.log("🔥 Admin Layout - Component Rendering Started");
    ...
    try {
        ...
        console.log("🔥 Admin Layout - Initial State:", {
            pathname,
            isAuthenticated,
            userRole: user?.role,
            statusLoading,
        ...
        useEffect(() => {
            ...
            if (user.role !== 'STORE') {
                console.log("Admin Layout - Wrong role, redirecting to home. User role:", user.role);
                toast.error("Access Denied", {
            ...
            console.log("Admin Layout - User is authenticated STORE user");
        }, [isAuthenticated, user?.id, user?.role, statusLoading, router]); // More specific dependencies

        // Check store application status
        useEffect(() => {
            console.log("Admin Layout - Store Status Check:", { 
                statusLoading, 
                applicationStatus: applicationStatus ? {
                    storeStatus: applicationStatus.storeStatus,
                    storeName: applicationStatus.storeName
                } : null,
                userRole: user?.role 
            });
            ...
            if (!statusLoading && applicationStatus && user?.role === 'STORE') {
                if (applicationStatus.storeStatus === 'PENDING' || 
                    ...
                    console.log("Admin Layout - Store not approved, redirecting to store-application. Status:", applicationStatus.storeStatus);
                    // Redirect to store application page
                    router.push('/store-application');
                    return;
                } else {
                    console.log("Admin Layout - Store is approved, allowing access. Status:", applicationStatus.storeStatus);
                }
            }
        }, [applicationStatus, statusLoading, user?.id, user?.role, isAuthenticated, router]); // More specific dependencies
        ...
        if (!applicationStatus) {
            console.log("Admin Layout - No application status received, showing error");
            return (
                <div className="min-h-screen bg-slate-50 flex items-center justify-center">
                    <div className="text-center">
        ...
        if (applicationStatus.storeStatus !== 'APPROVED') {
            console.log("Admin Layout - Store not approved, status:", applicationStatus.storeStatus);
            return null;
        }

        console.log("🔥 Admin Layout - Rendering main layout");
                            ...
                            <div className={`bg-white/5 rounded-3xl border border-white/5 relative overflow-hidden transition-all ${sidebarOpen ? 'p-6' : 'p-2 w-12 h-12 flex items-center justify-center'}`}>
                                ...
                                {sidebarOpen && (
                                    <Button 
                                        ...
                                        onClick={() => {
                                            console.log("🔥 Admin Layout - Logout clicked");
                                            logout();
